...
3. 送出時將 `SPI_TYP` 以 `Int` 帶入 `storeDitch`
4. API 回傳後，以 `String` 接收回傳值
5. 檢視或再次編輯時，以回傳值回填 UI

### 5.2 編輯流程
1. 進入編輯模式時，優先使用資料內的 `SPI_TYP`
2. 若沒有 `SPI_TYP`，則自動取「起點」的 `SPI_TYP`
3. UI 顯示對應文字
4. 送出時仍以 `SPI_TYP` 上傳

### 5.3 檢視流程
1. 檢視頁的 `tvSpiTyp` 使用 API 回傳的 `SPI_TYP` 填入
...
2. 離線新增、離線編輯、離線草稿恢復都要保留 `SPI_TYP`
