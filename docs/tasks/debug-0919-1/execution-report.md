...
| Full local suite `:app:testDebugUnitTest :app:assembleDebug :app:connectedDebugAndroidTest --no-daemon` | PASS on rerun | First run had one unrelated `MainShellActivityTest` focus timeout; subsequent complete `:app:connectedDebugAndroidTest --no-daemon` passed in 2m14s. Issue `ISS-debug-0919-1-001` is resolved as transient environment flake. |
