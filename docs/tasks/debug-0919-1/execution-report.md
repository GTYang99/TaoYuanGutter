...
| Full local suite `:app:testDebugUnitTest :app:assembleDebug :app:connectedDebugAndroidTest --no-daemon` | NOT VERIFIED | One unrelated `MainShellActivityTest` case hit Espresso `RootViewWithoutFocusException`; immediate isolated rerun passed. Tracked as environment issue `ISS-debug-0919-1-001`. |
